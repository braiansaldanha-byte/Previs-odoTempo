        ...
        function initMap() {
            try {
                ...
                console.warn('Leaflet não inicializado (possível falta de biblioteca):', e);
            }
        }

        function createCloudIcon(size = 72) {
            return L.divIcon({
                className: 'leaflet-div-icon cloud-marker',
                html: '☀️',
                iconSize: [size, size],
        ...
        function updateMap(lat, lon, isUser = false, locationInfo = null) {
            if (!map) return;
            const mapEl = document.getElementById('map');
            if (mapEl.classList.contains('hidden')) mapEl.classList.remove('hidden');

            const target = [lat, lon];
            if (isUser) {
                if (!userMarker) {
                    userMarker = L.marker(target, { icon: createCloudIcon(72) }).addTo(map);
                } else {
                    userMarker.setLatLng(target);
                    userMarker.setIcon(createCloudIcon(72));
                }
                const popupText = `Você${locationInfo ? ' — ' + (locationInfo.neighbourhood ? locationInfo.neighbourhood + ', ' : '') + (locationInfo.name || '') + (locationInfo.state ? ' - ' + locationInfo.state : '') : ''}`;
                userMarker.bindPopup(popupText);
            } else {
                if (!placeMarker) {
                    placeMarker = L.marker(target, { icon: createCloudIcon(72) }).addTo(map);
                } else {
                    placeMarker.setLatLng(target);
                    placeMarker.setIcon(createCloudIcon(72));
                }
                const popupText = `${locationInfo && (locationInfo.name || locationInfo.city) ? (locationInfo.name || locationInfo.city) : 'Local'}${locationInfo && locationInfo.state ? ', ' + locationInfo.state : ''}${locationInfo && locationInfo.neighbourhood ? ' — Bairro: ' + locationInfo.neighbourhood : ''}`;
                placeMarker.bindPopup(popupText);
            }

            try { map.invalidateSize(); } catch(e){}

            const markers = [].concat(userMarker ? [userMarker] : [], placeMarker ? [placeMarker] : []);
            if (markers.length === 0) {
        ...
        async function searchByCity() {
            ...
            try {
                ...
                await fetchWeatherDataByCoords(geo.lat, geo.lon, false, { name: geo.name, state: geo.state, country: geo.country });
            } catch (err) {
                showLoading(false);
        ...
        function getLocationWeather() {
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    await fetchWeatherDataByCoords(latitude, longitude, true);
                },
                (error) => {
                    showLoading(false);
        ...
        async function fetchWeatherDataByCoords(lat, lon, isUser = false, providedLocationInfo = null) {
            try {
                showLoading(true);
                hideError();
                ...
                let locationInfo = providedLocationInfo || null;
                try {
                    const owm = await reverseGeocodeOWM(lat, lon);
                    if (owm) locationInfo = { ...owm, ...locationInfo };
                ...
                try { updateMap(lat, lon, isUser, locationInfo); } catch(e) { console.warn(e); }
